...
namespace vkren
{
  ...
  {
    return StorageImage::Create(info.Image.Format, info.Image.Type, info.Image.Extent, info.View.ViewComponentMapping, info.Image.LayerCount, info.Image.MipmapLevels, info.Image.Copiable, info.Image.Flags, info.Image.Tiling, info.Image.SampleCount);
  }

  StorageImage StorageImage::Create(const VkFormat& format, const VkImageType& type, const VkExtent3D& extent, const VkComponentMapping& view_component_mapping, const uint32_t& layer_count, const uint32_t& mipmap_levels, bool copiable, const VkImageCreateFlags& flags, const VkImageTiling& tiling, const VkSampleCountFlagBits& sample_count)
  {
    StorageImage image;
    image.CreateImage(format, type, extent, (copiable ? VK_IMAGE_USAGE_TRANSFER_SRC_BIT : 0) | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, layer_count, mipmap_levels, flags, tiling, sample_count);
