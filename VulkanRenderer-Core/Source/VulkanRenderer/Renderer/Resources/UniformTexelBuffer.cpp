...
namespace vkren
{
  ...
  UniformTexelBuffer UniformTexelBuffer::Create(const VkDeviceSize& size, bool copiable, const VkFormat& format, const VkDeviceSize& range, const VkDeviceSize& offset)
  {
    ...
    CORE_ASSERT(buffer.m_Usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT, "[VULKAN/SYSTEM] To create the buffer's view it needs to be a texel buffer");
    VkFormatFeatureFlags formatFeatures = Renderer::GetDevice().GetFormatProperties(format).bufferFeatures;
    if (buffer.m_Usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT)
      CORE_ASSERT(formatFeatures & VK_FORMAT_FEATURE_UNIFORM_TEXEL_BUFFER_BIT, "[VULKAN/SYSTEM] The specified format cannot be used for uniform texel buffers");
    if (buffer.m_Usage & VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT)
      CORE_ASSERT(formatFeatures & VK_FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_BIT, "[VULKAN/SYSTEM] The specified format cannot be used for storage texel buffers");
