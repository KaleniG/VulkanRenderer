...
namespace vkren
{
  ...
  {
    CORE_ASSERT(PipelineInputAssemblyState::CheckResetPrimitiveToTopology(topology, reset_primitive), "[SYSTEM/VULKAN] Cannot reset primitives with this input primitive topology");
  ...
  PipelineRasterizationState::PipelineRasterizationState(bool enable_rasterization, bool depth_clamp, PolygonMode polygon_mode, CullMode cull_mode, FrontFace front_face, float line_width)
  {
...

    {
      ...
      switch (front_face)
      {
      case vkren::FrontFace::Clockwise:         vkfrontFace = VK_FRONT_FACE_CLOCKWISE; break;
      case vkren::FrontFace::CounterClockwise:  vkfrontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE; break;
      }
  ...
  {
    CORE_ASSERT(PipelineDynamicStates::CheckRepeat(state), "[SYSTEM/VULKAN] Do not repeat dynamic states");
  ...
  void PipelineLayoutStructure::AddDescriptorSetLayout(const Ref<DescriptorSetLayout>& layout)
  {
  ...
  Ref<PipelineLayout> PipelineLayout::Create(const PipelineLayoutStructure& structure)
  {
    Ref<PipelineLayout> layout = CreateRef<PipelineLayout>();
